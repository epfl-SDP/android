...
class FlowsTest {
  ...
  fun given_gameFlow_when_incrementalUpdate_then_endsInCorrectState() = runTest {
    ...
    val one = game.toExtendedNotation()
    game.play { Position(0, 1) += Delta(0, 2) }
    val two = game.toExtendedNotation()
    val games = flowOf(one, two)

    assertThat(games.mapToGame().map { it.toExtendedNotation() }.toList()).containsExactly(one, two)
  }

  @Test
  fun given_gameFlow_when_nonIncrementalUpdate_then_endsInCorrectState() = runTest {
    val one = Game.create().play { Position(0, 6) += Delta(0, -2) }.toExtendedNotation()
    val two = Game.create().play { Position(0, 5) += Delta(0, -2) }.toExtendedNotation()
    val games = flowOf(one, two)
